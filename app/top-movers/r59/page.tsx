...
export default function Report59Page() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      // Sort by date column descending (latest first)
    ...
    <div className="p-4 md:p-6 space-y-4">
      ...
      <div className="bg-[#13161e] border border-white/[0.07] rounded-xl overflow-hidden" style={{ borderLeft: '3px solid #4B9EF5' }}>
        {loading ? (
          ...
          <div className="py-16 text-center">
            <p className="text-[13px] text-white/30">{rawRows.length > 0 ? 'ไม่พบผลลัพธ์ที่ตรงกัน' : 'ไม่พบข้อมูล'}</p>
          </div>
        ) : (
      ...
      <p className="text-[10px] text-white/20 text-right">
        แหล่งข้อมูล: ก.ล.ต. (SEC) · Form 59-2 · {rows.length > 0 ? `${rows.length} รายการ` : ''}
      </p>
    </div>
  );
}
