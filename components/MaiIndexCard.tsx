...
export default function MaiIndexCard() {
  const [data, setData] = useState<IndexData | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch('/api/prices?symbols=%5EMAIX.BK')
      .then(r => r.json())
      .then(json => {
        const d = json.prices?.['^MAIX.BK'];
        if (d) setData(d);
        setLoading(false);
      })
  ...
  return (
    <div className="bg-[#13161e] border border-white/[0.07] rounded-xl p-4 min-w-[160px] flex-shrink-0">
      <p className="text-[10px] font-semibold uppercase tracking-wider text-white/35 mb-1.5">MAI Index</p>
      {loading ? (
        <>
