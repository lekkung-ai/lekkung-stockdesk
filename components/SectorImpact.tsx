...
function SectorRow({ item, positive }: { item: SectorImpactItem; positive: boolean }) {
  ...
  return (
    <div className="flex items-center gap-2 py-[5px] border-b border-white/[0.04] last:border-0">
      <span className="text-[13px] font-bold text-white/70 w-[70px] shrink-0">
        ${item.sector}
      </span>
      <span className="text-[11px] tabular-nums text-white/25 w-[32px] text-right shrink-0">
        {item.stockCount}
      </span>
...
export default function SectorImpact() {
  ...
  const gainers = data?.sectorImpacts.filter(x => x.impact > 0) ?? [];
  const losers  = [...(data?.sectorImpacts.filter(x => x.impact < 0) ?? [])].reverse();
      ...
      <div className="flex items-center justify-between mb-3">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-wider text-white/25 mb-0.5">
            Sector Impact วันนี้
          </p>
          <p className="text-[12px] text-white/30">SET100 · Impact ต่อดัชนีรายกลุ่ม</p>
        </div>
        {data && (gainers.length > 0 || losers.length > 0) && (
          ...
          >
            ...
            Net: {netImpact >= 0 ? '+' : ''}{fmt(netImpact, 2)} จุด
          </div>
        )}
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-5 gap-y-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 py-1 border-b border-white/[0.07] mb-0.5">
              <span className="text-[10px] text-white/20 w-[70px] shrink-0">กลุ่ม</span>
              <span className="text-[10px] text-white/20 w-[32px] text-right shrink-0">หุ้น</span>
              <span className="text-[10px] text-white/20 flex-1 text-right">impact</span>
            </div>
            {gainers.length === 0 ? (
              <p className="text-[13px] text-white/25 py-3 text-center">ไม่มีกลุ่มหนุน</p>
            ) : (
          ...
          <div>
            ...
            {losers.length === 0 ? (
              <p className="text-[13px] text-white/25 py-3 text-center">ไม่มีกลุ่มกด</p>
            ) : (
