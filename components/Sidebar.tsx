  ...
  return (
    <aside
      className={[
        ...
        'w-[75vw] max-w-[280px] md:w-[60px] lg:w-[220px] flex-shrink-0 flex flex-col',
    ...
    >
      ...
      <div className="h-14 flex items-center justify-between px-3 lg:px-4 border-b border-white/[0.07] flex-shrink-0">
        <span className={`font-bold text-[15px] text-white tracking-tight ${open ? 'block' : 'hidden'} md:hidden lg:block`}>
          StockDesk
        </span>
        <span className={`font-bold text-[13px] text-white tracking-tight ${open ? 'hidden' : 'block'} md:block lg:hidden`}>
          SD
        </span>
        <button
          onClick={onClose}
      ...
      <nav className="flex-1 overflow-y-auto py-3 px-1 lg:px-2 space-y-4">
        {NAV_GROUPS.map(group => (
          <div key={group.section}>
            <p className={`px-2 mb-1 text-[10px] font-semibold uppercase tracking-widest text-white/25 ${open ? 'block' : 'hidden'} md:hidden lg:block`}>
              {group.section}
            </p>
                ...
                return (
                  <Link
                    ...
                    className={[
                      ...
                      item.indent
                        ? `${open ? 'pl-7 pr-2' : 'px-2'} md:px-2 lg:pl-7 lg:pr-2`
                        : 'px-2',
                      active
                  ...
                  >
                    ...
                    <span className={`truncate text-[12.5px] ${open ? 'block' : 'hidden'} md:hidden lg:block`}>
                      {label}
                    </span>
                  </Link>
                );
      ...
      <div className="p-1 lg:p-2 border-t border-white/[0.07] flex-shrink-0">
        <Link
          href="/settings"
          title="Settings"
          className={`flex items-center gap-2 px-2 py-2.5 md:py-[6px] rounded-lg text-[12.5px] text-white/45 hover:text-white/75 hover:bg-white/[0.05] transition-colors ${open ? 'justify-start' : 'justify-center'} md:justify-center lg:justify-start`}
        >
          <Settings size={13} />
          <span className={`${open ? 'block' : 'hidden'} md:hidden lg:block`}>Settings</span>
        </Link>
      </div>
    </aside>
  );
